...
{% else %}
    {{ raise('An error occurred: unexpected condition') }}
{% endif %}
...
{% if contract.version == "2016" %}
    4. יובהר כי במקרה של שוכרים־שותפים ימציא כל אחד מהם ערב כאמור בחלופה הראשונה בסעיף 15.1 וכי במקרה של הפרה, יעשה בעל הדירה שימוש ראשית בביטחונות של השותף המפר.
{% elif contract.version == "2023" %}
